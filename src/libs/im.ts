...
export const iminit_TIM = (
  ...
  callbacks: {
    onImLogin: (isImLogin: boolean) => void
    onSDKReady: (eventName: string) => void
    onSDKNotReady: (eventName: string) => void
    onMessage: (msg: any, msgList: any) => void
    onMessageReaded: (msg: any, msgList: any) => void
    onUpdateRoomNum: (num: number) => void
    onConversationList: (list: any) => void
  }
) => {
  let isImLogin = false
  const { appId, userId, userSig } = params
  const { onImLogin, onMessage, onMessageReaded, onSDKReady, onSDKNotReady } = callbacks
  const options = {
  ...
  tim.on(TIM.EVENT.MESSAGE_READ_BY_PEER, (event) => {
    ...
    onMessageReaded(msgarr, newMsgForm)
  ...
  tim.on(TIM.EVENT.CONVERSATION_LIST_UPDATED, (event) => {
    ...
    isImLogin && initRecentContactList(tim, callbacks)
...
const initRecentContactList = (
  ...
  const { onUpdateRoomNum, onConversationList } = callbacks
...
const loginIm_TIM = (
  tim: any,
  params: { userId: string; userSig: string; isImLogin?: boolean },
  callbacks: {
    onImLogin: (isImLogin: boolean) => void
    onUpdateRoomNum: (num: number) => void
    onConversationList: (list: any) => void
  ...
  const { userId, userSig, isImLogin } = params
  const { onImLogin } = callbacks
  if (!!userId && !isImLogin) {
